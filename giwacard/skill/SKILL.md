...
| `INSUFFICIENT_AVAILABLE_BALANCE` | no | `available` cannot cover the requested cap. `details` carries `available` and `required`, in base units. | Mint a smaller card, `cancel_card` an unused one to free escrow, or ask the owner to run `giwacard deposit <amount>`. Note that gUSD claimed by `giwacard faucet` lands in the owner's *wallet* and does not back cards until it is deposited into the vault. Do **not** file an approval request — approval does not create funds. |
