...
- KTD-9. **Skema x402 kustom bersandar pada charge vault — bukan Permit2.** Permit2 `SignatureTransfer` menarik token dari saldo penanda tangan, sedangkan gUSD berada di escrow dalam vault dan session EOA tidak memegang gUSD — jalur itu tidak bisa menyelesaikan pembayaran. Karena `charge` vault sudah memindahkan dana ke merchant, Permit2 juga redundan. Skema yang dipakai: merchant menjawab `402` dengan requirements (alamat merchant, jumlah, token, alamat vault); MCP server menyubmit `CardVault.charge(cardId, amount)` dari session EOA lalu mengirim `X-PAYMENT` berisi tx hash + `cardId`; facilitator (menyatu di service merchant, tanpa EOA berdana) memverifikasi event `Charged` cocok pada alamat vault, jumlah, dan merchant, lalu mengembalikan `200` + `PAYMENT-RESPONSE`. Permit2/EIP-3009 = roadmap interoperabilitas.
...
- KTD-14. **Dashboard = Next.js `frontend/` yang ada, scope minimal**, dengan wallet browser untuk menandatangani approve. Owner menghubungkan wallet browser (discovery EIP-6963 + `walletClient` viem); dashboard meminta signature EIP-712 owner dan menyubmit tx mint-on-approve dari wallet itu (wallet owner yang bayar gas). Server dashboard tidak pernah menyentuh material kunci. Baca `node_modules/next/dist/docs/` dulu (Next 16.2.12 + React 19 + Tailwind v4 + React Compiler aktif). Auth: localhost-only MVP, tanpa SIWE.
